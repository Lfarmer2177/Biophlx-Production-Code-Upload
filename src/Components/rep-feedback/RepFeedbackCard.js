...
export default function RepFeedbackCard({ reps }) {
  ...
  const band = (label, data, available, number) => {
    ...
    return <View key={number} style={{ marginTop: 12, padding: 16, borderRadius: 14, backgroundColor: dark ? '#303034' : '#F5F5F7' }}>
      ...
      <Text style={{ color: muted, lineHeight: 23 }}>Range of motion: {available ? metric(data?.rom, '°') : '—'}{'\n'}Rep duration: {available ? metric(data?.tut, 's') : '—'}{'\n'}Velocity: {available ? metric(data?.velocity, 'm/s') : '—'}</Text>
      <Text style={{ color: muted, marginTop: 8 }}>{!available ? 'Waiting for this band’s rep measurement.' : rating == null ? 'No valid range measurement.' : 'ROM score: 0 through 90°, 50 above 90°, 100 above 120°.'}</Text>
    </View>;
  };
  return <View style={{ padding: 20, borderRadius: 20, backgroundColor: dark ? '#202024' : '#FFFFFF', borderWidth: 1, borderColor: dark ? '#414147' : '#E4E4E7', marginVertical: 12 }}>
    ...
    {!rep ? <Text style={{ color: muted, marginTop: 12 }}>Complete a rep to see a separate rating and metrics for each band.</Text> : <>
      ...
      <Text style={{ color: ink, fontWeight: '700', marginTop: 16 }}>Combined summary score: {score(rep) == null ? 'Not rated' : `${score(rep)} / 100`}</Text>
      <Text style={{ color: muted, marginTop: 8, lineHeight: 20 }}>The saved score is the average of both band ratings. Ratings use the same ROM scoring rule for every exercise; they are not an overall form or safety assessment.</Text>
    </>}
